    ...
    <section className="page" style={{ position: "relative", zIndex: 1 }}>
      <div className="cv-grid">
        <div className="cv-sidebar" style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {cvTabKeys.map((key, i) => (
            <button key={key} onClick={() => setCvTab(key)} style={{ textAlign: "left", display: "flex", alignItems: "center", justifyContent: "space-between", background: cvTab === key ? COLORS.accentSoft : COLORS.bgPanel, border: `1px solid ${cvTab === key ? COLORS.accent : COLORS.border}`, borderRadius: "10px", padding: "14px 16px", cursor: "pointer", color: cvTab === key ? COLORS.accentBright : COLORS.textMuted, fontFamily: "var(--font-jbmono), monospace", fontSize: "13px", fontWeight: 600, transition: "all 0.2s ease" }}>
