      ...
      <div style={{ borderRadius: "18px", overflow: "hidden" }}>
        <div style={{ padding: "22px 28px 0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "18px" }}>
            <span style={{ fontFamily: "var(--font-jbmono), monospace", fontSize: "12px", color: COLORS.textFaint }}>&gt;_ ~/logs/{p.id}</span>
            <span style={{ fontFamily: "var(--font-jbmono), monospace", fontSize: "10.5px", color: COLORS.accentBright, border: `1px solid ${COLORS.accent}50`, borderRadius: "100px", padding: "4px 10px" }}>[ {p.tag} ]</span>
          </div>
          <h3 style={{ margin: "0 0 4px", color: COLORS.text, fontSize: "24px", fontFamily: "var(--font-sora), sans-serif", fontWeight: 700 }}>{p.name}</h3>
          <p style={{ margin: "0 0 16px", color: COLORS.accentBright, fontSize: "13px", fontFamily: "var(--font-jbmono), monospace" }}>{copy.subtitle}</p>
          <p style={{ color: COLORS.textMuted, fontSize: "14px", lineHeight: "1.7", marginBottom: "20px" }}>{copy.description}</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "22px" }}>
            {p.tech.map((tech) => (
              <span key={tech.name} style={{ display: "inline-flex", alignItems: "center", gap: "6px", background: COLORS.bgPanelAlt, border: `1px solid ${COLORS.border}`, borderRadius: "6px", padding: "5px 10px", fontSize: "12px", fontFamily: "var(--font-jbmono), monospace", color: COLORS.textMuted }}>
